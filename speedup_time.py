...
def plot(names, object_labels, group_labels, data, lengend, output_path,
    ...
    for it, name in enumerate(names):
        ...
        ax.set_ylim(0, max_ylims[it])
        ax.set_yticks(np.arange(0, max_ylims[it], ysteps[it]))
...
def draw_figure(input_path, output_path, max_ylims, ysteps):
    names, header, labels, all_data = read_data(input_path)

    output_path = "figures/" + "speedup_graphsage_time.pdf"
    plot(names, labels, header, all_data, None, output_path, max_ylims, ysteps,
         "SAGE")


def draw_figure2(input_path, output_path, max_ylims, ysteps):
    names, header, labels, all_data = read_data(input_path)

    output_path = "figures/" + "speedup_gat_time.pdf"
    plot(names, labels, header, all_data, None, output_path, max_ylims, ysteps,
         "GAT")


if __name__ == "__main__":
    draw_figure("data/speedup_graphsage_time.csv", "figures",
                [25, 121, 151, 151], [10, 50, 60, 60])
    draw_figure2("data/speedup_gat_time.csv", "figures", [25, 121, 151, 151],
                 [10, 50, 60, 60])

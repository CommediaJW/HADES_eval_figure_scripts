...
def plot(plt, title, names, data, output_path, max_xlim, xstep, min_ylim,
    ...
    plt.set_title(title, fontsize=font_size, y=-0.62, fontweight="bold")
    plt.tick_params(
        axis="both",
        which="major",
        labelsize=font_size,
...
if __name__ == "__main__":
    ...
    font_size = 20
    tick_space_len = 1
    draw_figure(plt.subplot(2, 2, 1), "PD-GraphSAGE",
    ...
    draw_figure(plt.subplot(2, 2, 2), "RD-GraphSAGE",
                "data/accuracy_time_graphsage_reddit.csv",
                "figures/accuracy_time_graphsage_reddit.pdf", 80, 40, 0, 1,
                0.5, True, False, 0.53, 0.95, 0.77, 45, 0.62, 0.93)
    draw_figure(plt.subplot(2, 2, 3), "PD-GAT",
    ...
    draw_figure(plt.subplot(2, 2,
                            4), "RD-GAT", "data/accuracy_time_gat_reddit.csv",
                "figures/accuracy_time_gat_reddit.pdf", 60, 30, 0.5, 1, 0.25,
                True, False, 0.77, 0.95, 0.86, 38.5, 0.79, 0.921)
    plt.legend(
        fontsize=font_size - 2,
        edgecolor="k",
        ncol=2,
        loc="upper center",
        bbox_to_anchor=(-0.35, 3.2),
    )
    print(f"[Note]Save to figures/accuracy_time.pdf")
    plt.savefig("figures/accuracy_time.pdf", bbox_inches="tight")
    plt.close("all")
